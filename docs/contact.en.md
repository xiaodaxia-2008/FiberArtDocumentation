---
comments: true
hide:
    - navigation
    - toc
    - footer
---

# Contact Us

You can contact us through the following ways:

- WeChat: xiaodaxia-1

    ![](./assets/wechat.png){width="250px"}

- Email: <fiberart@myshawn.com>
- QQ [674936832](mqqwpa://im/chat?chat_type=wpa&uin=674936831)  

    ![](./assets/qq.png){width="250px"}

- FiberArt Software Exchange QQ Group: [959826007](https://qm.qq.com/q/4qmMJzRYt2)  

    ![](./assets/qq_group.jpg){width="250px"}

You can also leave a message directly below.

After you submit a [Trial Application](./trial/index.md), we will obtain your phone number, email, and other information and will contact you promptly.
