...
初次打开软件，显示的界面如下图所示。当前版本的完整工作流请先阅读[当前版本工作流](current_workflow.md)；本页的截图主要用于了解基本布局。

![FiberArt Initial Window](./images/fiberart_window.png)

### 菜单栏

菜单栏是软件所有功能的入口，单击每个菜单可以下拉打开子菜单。规划器、仿真后处理、标定、碰撞检测、URDF 生成器等功能可以通过*功能*菜单显示或隐藏。
...
也可以直接阅读[当前版本工作流](current_workflow.md)，了解设备配置、运动学、标定和程序导出的新流程。
